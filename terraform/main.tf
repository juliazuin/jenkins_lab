...
resource "aws_instance" "jenkins" {
  ...
  tags = {
    Name = "Julia-jenkins-${count.index}"
  }
  vpc_security_group_ids = [aws_security_group.julia_acessos_jenkins.id]
}


resource "aws_security_group" "julia_acessos_jenkins" {
...
output "jenkins" {
  value = [
    for key, item in aws_instance.jenkins :
